...
If a CodeGraph MCP server is wired into this environment (tools prefixed \`codegraph_*\`), prefer it for structural code questions. CodeGraph is a tree-sitter-parsed knowledge graph of every symbol, edge, and file. Reads are sub-millisecond and return structural information grep cannot. If the \`codegraph_*\` tools aren't available in this session, fall back to native grep/read — don't surface "missing tool" errors to the user unprompted.
